...
def calc_chunksize(num_dicts, min_chunksize=4, max_chunksize=2000, max_processes=128):
    ...
    # for small files (containing few dicts) we want small chunksize to ulitize all available cores but never less  # noqa: E501
...
class WANDBLogger(BaseMLLogger):
    ...
    def end_run(cls):
        if not cls.disable_logging and cls.experiment is not None:
            ...
            if cls.log_checkpoint:
                logger.info(
                    f"Finalizing the experiment. Uploading checkpoints from {cls.save_dir}"  # noqa: E501
                )  # noqa: E501
    ...
    def disable(cls):
        logger.warning(
            f"ML Logging is turned off. No parameters, metrics or artifacts will be logged to {cls.experiment.url}"  # noqa: E501
        )  # noqa: E501
...
def convert_iob_to_simple_tags(preds, spans, probs):
    ...
    if contains_named_entity and len(simple_tags) == 0:
        raise Exception(
            "Predicted Named Entities lost when converting from IOB to simple tags. Please check the format"  # noqa: E501
            "of the training data adheres to either adheres to IOB2 format or is converted when "  # noqa: E501
...
def all_gather_list(data, group=None, max_size=16384):
    ...
    if enc_size + SIZE_STORAGE_BYTES > max_size:
        raise ValueError(
            f"encoded data exceeds max_size, this can be fixed by increasing buffer size: {enc_size}"  # noqa: E501
        )  # noqa: E501
