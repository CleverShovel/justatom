...
def sample_to_features_qa_Natural_Questions(
    ...
    # Generates a numpy array of shape (max_answers, 2) where (i, 2) indexes into the start and end indices  # noqa: E501
    # of the ith answer. The array is filled with -1 since the number of answers is often less than max_answers  # noqa: E501
    ...
    # (question_len_t + passage_len_t + n_special_tokens). This may be less than max_seq_len but will not be greater  # noqa: E501
    # than max_seq_len since truncation was already performed when the document was chunked into passages  # noqa: E501
    ...
    if tokenizer.is_fast:
        ...
        if n_tokens_encoded != n_tokens_with_metadata:
            ...
            logger.error(
                ...
                f"tokenize_with_metadata: {sample.tokenized['question_tokens'] + sample.tokenized['passage_tokens']}"  # noqa: E501
            )  # noqa: E501
    else:
    ...
    # seq_2_start_t is the index of the first token in the second text sequence (e.g. passage)  # noqa: E501
    ...
    # The XLM-Roberta tokenizer generates a segment_ids vector that separates the first sequence from the second.  # noqa: E501
    # However, when this is passed in to the forward fn of the Roberta model, it throws an error since  # noqa: E501
    # Roberta has only a single token embedding (!!!). To get around this, we want to have a segment_ids  # noqa: E501
    # vec that is only 0s
    ...
    # start of word and spec_tok_mask are not actually needed by model.forward() but are needed for model.formatted_preds()  # noqa: E501
...
def generate_labels(
    ...
    ["no_answer", "yes", "no", "span"] and this is what answer_type_list should look like"""  # noqa: E501

    # Note here that label_idxs get passed to the QuestionAnsweringHead and answer_types get passed to the text  # noqa: E501
    # classification head. label_idxs may contain multiple start, end labels since SQuAD dev and test sets  # noqa: E501
    # can have multiple annotations. By contrast, Natural Questions only has one annotation per sample hence  # noqa: E501
...
def create_samples_qa_Natural_Question(
    ...
    for passage_span in passage_spans:
        ...
        # passage_offsets will be relative to the start of the passage (i.e. they will start at 0)  # noqa: E501
        # TODO: Is passage offsets actually needed? At this point, maybe we only care about token level  # noqa: E501
...
def chunk_into_passages(doc_offsets, doc_stride, passage_len_t, doc_text):
    ...
    that is formed when chunking a document using a sliding window approach."""  # noqa: E501
    ...
    while True:
        ...
        # If passage_end_t points to the last token in the passage, define passage_end_c as the length of the document  # noqa: E501
        if passage_end_t >= doc_len_t - 1:
            passage_end_c = len(doc_text)

        # Get document text up to the first token that is outside the passage. Strip of whitespace.  # noqa: E501
