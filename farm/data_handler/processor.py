...
class Processor(ABC):
    ...
    def load(
        ...
        **kwargs,
        ...
        logger.debug(
            f"Got more parameters than needed for loading {processor_name}: {unused_args}. "  # noqa: E501
    ...
    def load_from_dir(cls, load_dir):
        ...
        if "lower_case" in config.keys():  # noqa: SIM118
            logger.warning(
                ...
                "If you used `custom_vocab` or `never_split_chars`, this won't work anymore."  # noqa: E501
            )  # noqa: E501
        ...
        # we have to delete the tokenizer string from config, because we pass it as Object  # noqa: E501
        del config["tokenizer"]

        processor = cls.load(tokenizer=tokenizer, processor_name=config["processor"], **config)  # noqa: E501
    ...
    def convert_from_transformers(
        ...
        if task_type == "question_answering":
            ...
            processor = NERProcessor(
                ...
                max_seq_len=max_seq_len,
                data_dir="data",
                metric="seq_f1",  # noqa: E501
                label_list=label_list,
            )
        else:
            raise ValueError(
                f"`task_type` {task_type} is not supported yet. "
                f"Valid options for arg `task_type`: 'question_answering', "  # noqa: E501
    ...
    def _dict_to_samples_and_features(self, dictionary: dict, all_dicts=None) -> [Sample]:  # noqa: E501
        raise NotImplementedError()
    ...
    def log_problematic(problematic_sample_ids):
        if problematic_sample_ids:
            ...
            logger.error(
                f"Unable to convert {n_problematic} samples to features. Their ids are : {problematic_id_str}"  # noqa: E501
            )  # noqa: E501
        ...
        for basket in self.baskets:
            ...
            try:
                basket.samples = self._dict_to_samples_and_features(
                    dictionary=basket.raw,  # noqa: E501
                    all_dicts=all_dicts,
    ...
    def dataset_from_dicts(self, dicts, indices=None, return_baskets=False):
        ...
        """  # noqa: E501
        # We need to add the index (coming from multiprocessing chunks) to have a unique basket ID  # noqa: E501
        ...
        for id_internal, d in enumerate(dicts):
            ...
            self.baskets.append(SampleBasket(raw=d, id_external=id_external, id_internal=id_internal))  # noqa: E501
...
class TextClassificationProcessor(Processor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :type kwargs: object
        """  # noqa: E501
        # TODO If an arg is misspelt, e.g. metrics, it will be swallowed silently by kwargs  # noqa: E501
        ...
        if metric and label_list:
            ...
            logger.info(
                "Initialized processor without tasks. Supply `metric` and `label_list` to the constructor for "  # noqa: E501
                "using the default task or add a custom task later via processor.add_task()"  # noqa: E501
            )  # noqa: E501
    ...
    def dataset_from_dicts(self, dicts, indices=None, return_baskets=False, debug=False):  # noqa: E501
        self.baskets = []
        # Tokenize in batches
        ...
        problematic_ids = set()
        logger.warning("Currently no support in Processor for returning problematic ids")  # noqa: E501
...
class TextPairClassificationProcessor(TextClassificationProcessor):
    ...
    def file_to_dicts(self, file: str) -> [dict]:
        ...
        # during tokenization inside TextClassificationProcessor only the "text" field is tokenized  # noqa: E501
        # To convert text pairs we combine text and text_b into a tuple, then the tokenizer takes care of  # noqa: E501
...
class RegressionProcessor(TextClassificationProcessor):
    ...
    def _set_label_scaling(self, dicts, file):
        ...
        # for regression with Mean Squared Error loss we need to have labels centered around 0 with unit variance  # noqa: E501
        train_labels = []
...
class TextPairRegressionProcessor(RegressionProcessor):
    ...
    def file_to_dicts(self, file: str) -> [dict]:
        ...
        # during tokenization inside RegressionProcessor only the "text" field is tokenized  # noqa: E501
        # To convert text pairs we combine text and text_b into a tuple, then the tokenizers takes care of  # noqa: E501
        # adding separator tokens and correct segment IDs
        for d in dicts:
            d["text"] = (d["text"], d["text_b"])
        return dicts
...
class InferenceProcessor(TextClassificationProcessor):
    ...
    def load_from_dir(cls, load_dir):
        ...
        processor = cls.load(tokenizer=tokenizer, processor_name="InferenceProcessor", **config)  # noqa: E501
        for task_name, task in config["tasks"].items():
            processor.add_task(name=task_name, metric=task["metric"], label_list=task["label_list"])  # noqa: E501
    ...
    def dataset_from_dicts(self, dicts, indices=None, return_baskets=False, debug=False):  # noqa: E501
        ...
        """  # noqa: E501
        # TODO remove this sections once tokenizers work the same way for slow/fast and our special tokenizers  # noqa: E501
        if not self.tokenizer.is_fast:
            ...
            problematic_ids = set()
            logger.warning(
                "Currently no support in InferenceProcessor for returning problematic ids"  # noqa: E501
            )  # noqa: E501
            dataset, tensornames = self._create_dataset()
    ...
    def _dict_to_samples(self, dictionary: dict, **kwargs) -> [Sample]:
        ...
        # truncate tokens, offsets and start_of_word to max_seq_len that can be handled by the model  # noqa: E501
        for seq_name in tokenized.keys():  # noqa: SIM118
            tokenized[seq_name], _, _ = truncate_sequences(
                seq_a=tokenized[seq_name],
                seq_b=None,  # noqa: E501
...
class NERProcessor(Processor):
    ...
    def dataset_from_dicts(self, dicts, indices=None, return_baskets=False, non_initial_token="X"):  # noqa: E501
        self.baskets = []
        ...
        for i in range(len(dicts)):
            ...
            # We construct a mask to identify the first token of a word. We will later only use them for predicting entities.  # noqa: E501
            # Special tokens don't count as initial tokens => we add 0 at the positions of special tokens  # noqa: E501
            ...
            # i.e. if inference, we need to populate the tokenized_dict so that formatted preds can align  # noqa: E501
            ...
            for task_name, task in self.tasks.items():
                try:
                    ...
                    labels_token = expand_labels(labels_word, initial_mask, non_initial_token)  # noqa: E501
                    ...
                    logger.warning(
                        f"[Task: {task_name}] Could not convert labels to ids via label_list!"  # noqa: E501
                ...
                # TODO change this when inference flag is implemented
                    ...
                    logger.warning(
                        f"[Task: {task_name}] Could not convert labels to ids via label_list!"  # noqa: E501
                        "\nIf your are running in *inference* mode: Don't worry!"  # noqa: E501
...
class BertStyleLMProcessor(Processor):
    ...
    def dataset_from_dicts(self, dicts, indices=None, return_baskets=False):
        ...
        if self.next_sent_pred:
            assert len(dicts) > 1, "Need at least 2 documents to sample random sentences from"  # noqa: E501
            ...
            # ...bert style
                ...
                raise NotImplementedError("next_sent_pred_style has to be 'sentence' or 'bert-style'")  # noqa: E501
        ...
        for sample in samples:
            features.append(self._create_labels(sample=sample, vocab_length=vocab_length))  # noqa: E501
    ...
    def _create_sequence_pairs_by_line(self, docs):
        ...
        labels = []
        for doc in docs:
            # create one sample for each sentence in the doc (except for the very last -> "nextSentence" is impossible)  # noqa: E501
        ...
        for idx in range(len(raw_pairs)):
            ...
            # We don't populate 'tokenized' here as we skiped the intermediate string token stage abeoce to improve the speed ...  # noqa: E501
            samples.append(
                Sample(
                    id=None,
                    clear_text={
                        "text_a": raw_pairs[idx][0],
                        "text_b": raw_pairs[idx][1],
                        "nextsentence_label": labels[idx],
                    },
                    tokenized={
                        "tokens": encoded_pairs.encodings[idx].tokens,  # noqa: E501
                        "start_of_word": start_of_word[idx],
                        "special_tokens_mask": encoded_pairs.encodings[idx].special_tokens_mask,  # noqa: E501
                        "offsets": encoded_pairs.encodings[idx].offsets,
                    },  # noqa: E501
                    features={
                        "input_ids": encoded_pairs.input_ids[idx],
                        "segment_ids": encoded_pairs.token_type_ids[idx],  # noqa: E501
                        "padding_mask": encoded_pairs.attention_mask[idx],  # noqa: E501
                    },
                )
            )
        return samples
    ...
    def _create_sequence_pairs_bert_style(self, docs):
        ...
        encoded_docs = []
        for doc in docs:
            encoded_sentences = self.tokenizer.batch_encode_plus(doc, add_special_tokens=False)  # noqa: E501
            # Create "Start of word mask"
        ...
        for enc_doc in encoded_docs:
            ...
            while i < len(enc_doc.encodings):
                ...
                if current_length >= max_num_tokens:
                    # split our list of sequences (=chunk) into two sequences and create a sample out of it  # noqa: E501
    ...
    def _create_sequence_pairs_no_next_sent(self, docs):
        ...
        # Tokenize + Encode masks
        # TODO fill up sequences rather than creating one-sentence-samples to make this more efficient  # noqa: E501
        encoded_pairs = self.tokenizer.batch_encode_plus(
            docs,
            max_length=self.max_seq_len,
            truncation=True,
    ...
    def _create_sample_bert_style(self, chunk, random_doc, max_num_tokens, prob_next_sentence=0.5):  # noqa: E501
        """
        ...
        if len(chunk) == 1:
            ...
            input_ids = self.tokenizer.build_inputs_with_special_tokens(
                token_ids_0=chunk[0].ids[:boundary],  # noqa: E501
                token_ids_1=chunk[0].ids[boundary:max_num_tokens],
            )
            ...
            start_of_word = [0] + chunk[0].start_of_word[:boundary] + [0] + chunk[0].start_of_word[boundary:max_num_tokens] + [0]  # noqa: E501
            ...
            # b) ... using random next sequence
                ...
                # We didn't use all of the segments in this chunk as we sampled a random sequence => put them back  # noqa: E501
            ...
            # Insert special tokens
            input_ids = self.tokenizer.build_inputs_with_special_tokens(
                token_ids_0=sequence_a.ids,  # noqa: E501
            ...
            # TODO make this general for other model types
            start_of_word = [0] + start_of_word_a + [0] + start_of_word_b[:target_b_length] + [0]  # noqa: E501
            padding_mask = [1] * len(input_ids)

            if len(input_ids) < self.max_seq_len:
                # Pad up to the sequence length. For certain models, the pad token id is not 0 (e.g. Roberta where it is 1)  # noqa: E501
    ...
    def _create_labels(self, sample, vocab_length) -> dict:
        ...
        input_ids, lm_label_ids = self._mask_random_words(
            sample.features["input_ids"],
            vocab_length,
            token_groups=sample.tokenized["start_of_word"],
        )  # noqa: E501
        ...
        # NSP label
        if self.next_sent_pred:
            # Convert is_next_label: Note that in Bert, is_next_labelid = 0 is used for next_sentence=true!  # noqa: E501
    ...
    def _mask_random_words(self, tokens, vocab_length, token_groups=None, max_predictions_per_seq=20):  # noqa: E501
        """
        ...
        # 2. Mask the first groups until we reach the number of tokens we wanted to mask (num_to_mask)  # noqa: E501
        for index_set in cand_indices:
            ...
            for index in index_set:
                ...
                try:
                    ...
                    logger.warning(
                        f"Cannot find token '{original_token}' in vocab. Using [UNK] instead"  # noqa: E501
                    )
    ...
    def estimate_n_samples(self, filepath, max_docs=500):
        ...
        empty_lines = sum(
            1 if line == "\n" else 0
            for line in open(filepath, encoding="utf-8")  # noqa: SIM115
        )  # noqa: SIM115, E501
        ...
        if self.next_sent_pred_style == "sentence":
            ...
            # Original BERT LM training (filling up sequence pairs with sentences until max_seq_len)  # noqa: E501
            # (This is a very rough heuristic, as we can only estimate the real number of samples AFTER tokenization)  # noqa: E501
            ...
            logging.info(
                f"Heuristic estimate of number of samples in {filepath} based on {len(dicts)} docs: {n_samples}"  # noqa: E501
            )  # noqa: E501
        else:
            raise NotImplementedError(
                f"No estimate logic for next_sent_pred_style={self.next_sent_pred_style} implemented"  # noqa: E501
            )  # noqa: E501
        return n_samples
...
class SquadProcessor(Processor):
    ...
    def dataset_from_dicts(self, dicts, indices, return_baskets=False):
        ...
        pre_baskets = [self.convert_qa_input_dict(x) for x in dicts]  # TODO move to input object conversion  # noqa: E501

        # Tokenize documents and questions
        baskets = tokenize_batch_question_answering(pre_baskets, self.tokenizer, indices)  # noqa: E501
        ...
        # Convert internal representation (nested baskets + samples with mixed types) to pytorch features (arrays of numbers)  # noqa: E501
        baskets = self._passages_to_pytorch_features(baskets, return_baskets)

        # Convert features into pytorch dataset, this step also removes potential errors during preprocessing  # noqa: E501
    ...
    def _split_docs_into_passages(self, baskets):
        ...
        for basket in baskets:
            ...
            # Calculate the number of tokens that can be reserved for the passage. This is calculated by considering  # noqa: E501
            # the max_seq_len, the number of tokens in the question and the number of special tokens that will be added  # noqa: E501
            # when the question and passage are joined (e.g. [CLS] and [SEP])
            passage_len_t = self.max_seq_len - len(basket.raw["question_tokens"][: self.max_query_length]) - n_special_tokens  # noqa: E501
            ...
            for passage_span in passage_spans:
                ...
                passage_start_of_word = basket.raw["document_start_of_word"][passage_start_t:passage_end_t]  # noqa: E501
                passage_tokens = basket.raw["document_tokens"][passage_start_t:passage_end_t]  # noqa: E501
                passage_text = basket.raw["document_text"][passage_start_c:passage_end_c]  # noqa: E501
                ...
                tokenized = {
                    ...
                    "question_tokens": basket.raw["question_tokens"][: self.max_query_length],  # noqa: E501
                    "question_offsets": basket.raw["question_offsets"][: self.max_query_length],  # noqa: E501
                    "question_start_of_word": basket.raw["question_start_of_word"][: self.max_query_length],  # noqa: E501
                }
    ...
    def _convert_answers(self, baskets):
        ...
        for basket in baskets:
            ...
            for num, sample in enumerate(basket.samples):  # noqa: B007
                ...
                # Initializing a numpy array of shape (max_answers, 2), filled with -1 for missing values  # noqa: E501
                ...
                if error_in_answer or (len(basket.raw["answers"]) == 0):
                    ...
                    # For all other cases we use start and end token indices, that are relative to the passage  # noqa: E501
                    for i, answer in enumerate(basket.raw["answers"]):
                        ...
                        answer_start_t = offset_to_token_idx_vecorized(basket.raw["document_offsets"], answer_start_c)  # noqa: E501
                        answer_end_t = offset_to_token_idx_vecorized(basket.raw["document_offsets"], answer_end_c)  # noqa: E501
                        # TODO remove after testing 'offset_to_token_idx_vecorized()'
                        # answer_start_t2 = offset_to_token_idx(doc_offsets, answer_start_c)  # noqa: E501
                        # answer_end_t2 = offset_to_token_idx(doc_offsets, answer_end_c)
                        # if (answer_start_t != answer_start_t2) or (answer_end_t != answer_end_t2):  # noqa: E501
                        #     pass
                        ...
                        if passage_len_t > answer_start_t >= 0 and passage_len_t >= answer_end_t >= 0:  # noqa: E501
                            # Then adjust the start and end offsets by adding question and special token  # noqa: E501
                            label_idxs[i][0] = self.sp_toks_start + question_len_t + self.sp_toks_mid + answer_start_t  # noqa: E501
                            label_idxs[i][1] = self.sp_toks_start + question_len_t + self.sp_toks_mid + answer_end_t  # noqa: E501
                        # If the start or end of the span answer is outside the passage, treat passage as no_answer  # noqa: E501
                        else:
                            label_idxs[i][0] = 0
                            label_idxs[i][1] = 0

                        ########## answer checking ##############################
                        # TODO, move this checking into input validation functions and delete wrong examples there  # noqa: E501
                        ...
                        if answer_start_t < 0 or answer_end_t >= passage_len_t:
                            ...
                            if answer_text not in doc_text:
                                logger.warning(
                                    ...
                                    f"Example will not be converted for training/evaluation."  # noqa: E501
                                )  # noqa: E501
                                error_in_answer = True
                                label_idxs[i][0] = -100  # TODO remove this hack also from featurization  # noqa: E501
                                label_idxs[i][1] = -100
                                break  # Break loop around answers, so the error message is not shown multiple times  # noqa: E501
    ...
    def _passages_to_pytorch_features(self, baskets, return_baskets):
        ...
        for basket in baskets:
            ...
            for num, sample in enumerate(basket.samples):  # noqa: B007
                ...
                # - Combines question_tokens and passage_tokens into a single vector called input_ids  # noqa: E501
                # - input_ids also contains special tokens (e.g. CLS or SEP tokens).
                # - It will have length = question_len_t + passage_len_t + n_special_tokens. This may be less than  # noqa: E501
                #   max_seq_len but never greater since truncation was already performed when the document was chunked into passages  # noqa: E501
                question_input_ids = sample.tokenized["question_tokens"]
                passage_input_ids = sample.tokenized["passage_tokens"]

                input_ids = self.tokenizer.build_inputs_with_special_tokens(
                    token_ids_0=question_input_ids,  # noqa: E501
                ...
                # The passage mask has 1 for tokens that are valid start or ends for QA spans.  # noqa: E501
                # 0s are assigned to question tokens, mid special tokens, end special tokens and padding  # noqa: E501
                # Note that start special tokens are assigned 1 since they can be chosen for a no_answer prediction  # noqa: E501
                ...
                len_check = len(input_ids) == len(padding_mask) == len(segment_ids) == len(start_of_word) == len(span_mask)  # noqa: E501
                id_check = len(sample_id) == 3
                label_check = return_baskets or len(sample.tokenized.get("labels", [])) == self.max_answers  # noqa: E501
                label_check2 = return_baskets or np.all(
                    sample.tokenized["labels"] > -99
                )  # labels are set to -100 when answer cannot be found  # noqa: E501
                if len_check and id_check and label_check and label_check2:
                    # - The first of the labels will be used in train, and the full array will be used in eval.  # noqa: E501
                    # - start_of_word and spec_tok_mask are not actually needed by model.forward() but are needed for  # noqa: E501
                    #   model.formatted_preds() during inference for creating answer strings  # noqa: E501
                    # - passage_start_t is index of passage's first token relative to document  # noqa: E501
                    feature_dict = {
                        "input_ids": input_ids,
                        "padding_mask": padding_mask,
                        "segment_ids": segment_ids,
                    ...
                    sample.features = [feature_dict]  # other processor's features can be lists  # noqa: E501
                else:
...
class NaturalQuestionsProcessor(Processor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # These are classification labels from Natural Questions. Note that in this implementation, we are merging  # noqa: E501
    ...
    def _dict_to_samples(self, dictionary: dict, all_dicts=None) -> [Sample]:
        ...
        dictionary_tokenized = self._apply_tokenization(dictionary, self.tokenizer, self.answer_type_list)[0]  # noqa: E501
        n_special_tokens = self.tokenizer.num_special_tokens_to_add(pair=True)
        ...
        # Downsample the number of samples with an no_answer label. This fn will always return at least one sample  # noqa: E501
    ...
    def _downsample(self, samples, keep_prob):
        # Downsamples samples with a no_answer label (since there is an overrepresentation of these in NQ)  # noqa: E501
        # This method will always return at least one sample. This is done so that we don't end up with SampleBaskets  # noqa: E501
        # with 0 samples
        ret = []
        for s in samples:
    ...
    def _downsample_unprocessed(self, dictionary):
        ...
        if len(annotations) == 1:
            ...
            # There seem to be cases where there is no answer but an annotation is given as a (-1, -1) long answer  # noqa: E501
            if self._check_no_answer(annotation):
                dictionary["document_text"] = " ".join(doc_tokens[: self.max_seq_len + randint(1, self.downsample_context_size)])  # noqa: E501
            else:
                ...
                start_threshold = min(long_answer_start, short_answer_start) - randint(1, self.downsample_context_size)  # noqa: E501
                start_threshold = max(0, start_threshold)
                end_threshold = max(long_answer_end, short_answer_end) + randint(1, self.downsample_context_size)  # noqa: E501
    ...
    def _prepare_dict(self, dictionary):
        ...
        ["yes", "no", "span", "no_answer"]"""  # noqa: E501
        ...
        for annotation in dictionary["annotations"]:
            ...
            if self._check_no_answer(annotation):
                continue
            sa_text, sa_start_c = self._unify_short_answers(annotation["short_answers"], doc_text, tok_to_ch)  # noqa: E501
            la_text, la_start_c = self._retrieve_long_answer(
                annotation["long_answer"]["start_token"],  # noqa: E501
                annotation["long_answer"]["end_token"],
                tok_to_ch,
                doc_text,
            )
            # Picks the span to be considered as annotation by choosing between short answer, long answer and no_answer  # noqa: E501
        ...
        if len(converted_answers) == 0:
            ...
            if answer_type == "none":
                answer_type = "span"
        # TODO: answer_type should be in answers since in NQ, each annotator can give either a span, no_answer, yes or no  # noqa: E501
        converted = {
    ...
    def _check_no_answer(annotation):
        if annotation["long_answer"]["start_token"] > -1 or annotation["long_answer"]["end_token"] > -1:  # noqa: E501
            return False
    ...
    def _check_no_answer_sample(sample):
        ...
        if first_answer["start_t"] < sample_tok["passage_start_t"]:
            return True
        if first_answer["end_t"] > sample_tok["passage_start_t"] + len(sample_tok["passage_tokens"]):  # noqa: E501
            return True
    ...
    def _unify_short_answers(self, short_answers, doc_text, tok_to_ch):
        ...
        span that contains all the answers"""  # noqa: E501
        ...
        answer_start_c, answer_end_c = self._convert_tok_to_ch(answer_start_t, answer_end_t, tok_to_ch, doc_text)  # noqa: E501
    ...
    def _check_valid_answer(self, sample):
        ...
        for answer in sample.clear_text["answers"]:
            ...
            if answer_indices != answer_text:
                raise ValueError(
                    f"""Answer using start/end indices is '{answer_indices}' while gold label text is '{answer_text}'"""  # noqa: E501
                )  # noqa: E501
        ...
        for question in questions:
            ...
            try:
                ...
                for answer in question["answers"]:
                    if (
                        "answer_type" in answer.keys()  # noqa: SIM118
                        and answer["answer_type"] in answer_types_list
                    ):  # noqa: SIM118, E501
            ...
            question_start_of_word = [int(x) for x in question_tokenized["start_of_word"]]  # noqa: E501

            # During inference, there is no_answer type. Also, question might be a str instead of a dict  # noqa: E501
    ...
    def _is_impossible_to_answer_type(self, qas):
        """Converts questions from having an is_impossible field to having an answer_type field"""  # noqa: E501
        new_qas = []
        for q in qas:
            answer_type = "span"
...
class TextSimilarityProcessor(Processor):
    ...
    def load_from_dir(cls, load_dir):
        ...
        config = json.load(open(processor_config_file))  # noqa: SIM115
        # init tokenizer
        query_tokenizer = Tokenizer.load(load_dir, tokenizer_class=config["query_tokenizer"], subfolder="query")  # noqa: E501
        passage_tokenizer = Tokenizer.load(load_dir, tokenizer_class=config["passage_tokenizer"], subfolder="passage")  # noqa: E501
    ...
    def file_to_dicts(self, file: str) -> [dict]:
        ...
        # shuffle dicts to make sure that similar positive passages do not end up in one batch  # noqa: E501
    ...
    def dataset_from_dicts(self, dicts, indices=None, return_baskets=False):
        ...
        # Take the dict and insert into our basket structure, this stages also adds an internal IDs  # noqa: E501
        ...
        # Convert features into pytorch dataset, this step also removes and logs potential errors during preprocessing  # noqa: E501
        dataset, tensor_names, problematic_ids, baskets = self._create_dataset(baskets)

        if problematic_ids:
            logger.error(
                f"There were {len(problematic_ids)} errors during preprocessing at positions: {problematic_ids}"  # noqa: E501
            )  # noqa: E501
    ...
    def _convert_queries(self, baskets):
        for basket in baskets:
            ...
            tokenized = {}
            features = [{}]
            # extract query, positive context passages and titles, hard-negative passages and titles  # noqa: E501
            if "query" in basket.raw:
                try:
                    ...
                    # tokenize query
                    tokenized_query = self.query_tokenizer.convert_ids_to_tokens(query_inputs["input_ids"])  # noqa: E501
    ...
    def _convert_contexts(self, baskets):
        for basket in baskets:
            if "passages" in basket.raw:
                try:
                    positive_context = list(filter(lambda x: x["label"] == "positive", basket.raw["passages"]))  # noqa: E501
                    if self.shuffle_positives:
                        random.shuffle(positive_context)
                    positive_context = positive_context[: self.num_positives]
                    hard_negative_context = list(
                        filter(
                            lambda x: x["label"] == "hard_negative",
                            basket.raw["passages"],
                        )
                    )  # noqa: E501
                    if self.shuffle_negatives:
                        random.shuffle(hard_negative_context)
                    hard_negative_context = hard_negative_context[: self.num_hard_negatives]  # noqa: E501

                    positive_ctx_titles = [passage.get("title", None) for passage in positive_context]  # noqa: E501
                    positive_ctx_texts = [passage["text"] for passage in positive_context]  # noqa: E501
                    hard_negative_ctx_titles = [passage.get("title", None) for passage in hard_negative_context]  # noqa: E501
                    hard_negative_ctx_texts = [passage["text"] for passage in hard_negative_context]  # noqa: E501

                    # all context passages and labels: 1 for positive context and 0 for hard-negative context  # noqa: E501
                    ctx_label = [1] * self.num_positives + [0] * self.num_hard_negatives
                    # featurize context passages
                    if self.embed_title:
                        # concatenate title with positive context passages + negative context passages  # noqa: E501
                        all_ctx = self._combine_title_context(
                            positive_ctx_titles, positive_ctx_texts
                        ) + self._combine_title_context(hard_negative_ctx_titles, hard_negative_ctx_texts)  # noqa: E501
                    else:
                        all_ctx = positive_ctx_texts + hard_negative_ctx_texts

                    # assign empty string tuples if hard_negative passages less than num_hard_negatives  # noqa: E501
                    all_ctx += [("", "")] * ((self.num_positives + self.num_hard_negatives) - len(all_ctx))  # noqa: E501
                    ...
                    ctx_segment_ids = np.zeros_like(ctx_inputs["token_type_ids"], dtype=np.int32)  # noqa: E501

                    # get tokens in string format
                    tokenized_passage = [self.passage_tokenizer.convert_ids_to_tokens(ctx) for ctx in ctx_inputs["input_ids"]]  # noqa: E501

                    # for DPR we only have one sample containing query and corresponding (multiple) context features  # noqa: E501
                    sample = basket.samples[0]
                    sample.clear_text["passages"] = positive_context + hard_negative_context  # noqa: E501
                    ...
                    sample.features[0]["passage_attention_mask"] = ctx_inputs["attention_mask"]  # noqa: E501
