...
class Document:
    ...
    # We use a custom init here as we want some custom logic. The annotations above are however still needed in order  # noqa: E501
    # to use some dataclass magic like "asdict()". See https://www.python.org/dev/peps/pep-0557/#custom-init-method
    # They also help in annotating which object attributes will always be present (e.g. "id") even though they  # noqa: E501
    # don't need to passed by the user in init and are rather initialized automatically in the init  # noqa: E501
    def __init__(
        self,
        content: str | pd.DataFrame,
        content_type: str = "text",
        ...
        if id_hash_keys is not None:  # noqa: SIM102
            if not set(id_hash_keys) <= set(allowed_hash_key_attributes):  # type: ignore
                raise ValueError(
                    f"You passed custom strings {id_hash_keys} to id_hash_keys which is deprecated. Supply instead a list"  # noqa: E501
                    f" of Document's attribute names that the id should be based on (e.g. {allowed_hash_key_attributes})."  # noqa: E501
    ...
    def _get_id(self, id_hash_keys: list[str] | None = None):
        ...
        if final_hash_key == "":
            raise ValueError(
                "Cant't create 'Document': 'id_hash_keys' must contain at least one of ['content', 'meta']"  # noqa: E501
            )  # noqa: E501
    ...
    def __str__(self):
        ...
        return f"<Document: id={self.id}, content='{self.content[:100]}{'...' if len(self.content) > 100 else ''}'>"  # noqa: E501
