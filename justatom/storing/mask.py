...
try:
    ...
    logger.debug(
        "Numba not found, replacing njit() with no-op implementation. Enable it with 'pip install numba'."  # noqa: E501
    )  # noqa: E501
    ...
    def _drop_duplicate_documents(self, documents: list[Document], index: str | None = None) -> list[Document]:  # noqa: E501
        ...
        for document in documents:
            if document.id in _hash_ids:
                logger.info(
                    "Duplicate Documents: Document with id '%s' already exists in index '%s'",  # noqa: E501
                    document.id,
                    index or self.index,
                )
                continue
